...
public sealed class ScrContainerCodec
{
    ...
    public ScrFileDocument Read(ReadOnlySpan<byte> source, string? sourceName = null)
    {
        ...
        ReadOffsetTable(source, ref offset, SaveMagic, document.SaveOffsets, document.Script, ScrOffsetEncoding.FileAbsolute, offset - checked((int)codeSize), resolveCodeLabels: true);
        ReadOffsetTable(source, ref offset, LayerMagic, document.LayerOffsets, document.Script, ScrOffsetEncoding.CodeRelative, 0, resolveCodeLabels: false);
        ScrSemanticPass.AttachInstructionLabels(document.Script, codeStart);
    ...
    public byte[] Write(ScrFileDocument document)
    {
        ...
        var codeStart = checked(Encoding.ASCII.GetByteCount(document.Header) + 4);
        ScrSemanticPass.MaterializeInstructionTargets(document.Script, codeStart);
        ...
        bool resolveCodeLabels)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (resolveCodeLabels && pc >= 0 && EnsureLabel(script, ScrLabelService.MakeOffsetLabel(pc), pc))
            {
