...
RECORD_SIZE = 12
MAGIC_YSTB = b"YSTB"
OPCODE_CONDITIONAL_BRANCH = 0x2D
OPCODE_SET_VALUE = 0x2C
OPCODE_SET_RUNTIME_VALUE = 0x36
OPCODE_MESSAGE_CONTROL = 0x60
OPCODE_MESSAGE_TEXT = 0x6A
OPCODE_MESSAGE_FORMAT = 0x6E
OPCODE_MESSAGE_COMMIT = 0x4F
MESSAGE_NEWLINE = "\n"
MESSAGE_PAGEBREAK = "\f"
MESSAGE_WAIT = "\v"
MESSAGE_CONTROL_ACTIVE_VALUES = {0, 1}
MESSAGE_CONTROL_COMPILE_VALUE = 1
CONTROL_SLOT_TO_MARKER = {
    11: MESSAGE_NEWLINE,
    12: MESSAGE_PAGEBREAK,
    13: MESSAGE_NEWLINE,
    14: MESSAGE_PAGEBREAK,
    15: MESSAGE_WAIT,
}
CONTROL_MARKER_TO_SLOT = {
    MESSAGE_NEWLINE: 11,
    MESSAGE_PAGEBREAK: 12,
    MESSAGE_WAIT: 15,
}
...
SET_VALUE_TEXT_SLOTS = {
    "es.CHAR.NAME": (33, 34, 35),
    "es.CHAR.NAME.MARK.SET": (33,),
}
EXTRACT_MODE_OP36_MARSHALED = "op36_marshaled"
...
def decode_message_text_instruction(inst: InstructionRecord, source_encoding: str) -> Optional[str]:
    if inst.opcode not in {OPCODE_MESSAGE_TEXT, OPCODE_MESSAGE_FORMAT}:
        return None
    if len(inst.args) != 1:
        return None
    arg = inst.args[0]
    if inst.opcode == OPCODE_MESSAGE_TEXT:
        if arg.slot != 0 or arg.kind != 0:
            return None
        try:
...
def decode_message_control_instruction(inst: InstructionRecord) -> List[str]:
    if inst.opcode != OPCODE_MESSAGE_CONTROL:
        return []
    out: List[str] = []
    for arg in inst.args:
        marker = CONTROL_SLOT_TO_MARKER.get(arg.slot)
        if marker is None:
            continue
        value = decode_literal_value(arg.payload)
        if value in MESSAGE_CONTROL_ACTIVE_VALUES:
            out.append(marker)
    return out


def split_message_text_parts(text: str) -> List[Tuple[str, str]]:
    out: List[Tuple[str, str]] = []
    buf: List[str] = []
    for ch in text:
        if ch in CONTROL_MARKER_TO_SLOT:
            if buf or not out:
                out.append(("text", "".join(buf)))
                buf.clear()
            out.append(("control", ch))
        else:
            buf.append(ch)
    if buf or not out:
        out.append(("text", "".join(buf)))
...
def make_message_control_instruction(marker: str, trace_value: int) -> InstructionRecord:
    slot = CONTROL_MARKER_TO_SLOT[marker]
    return InstructionRecord(
        index=0,
        opcode=OPCODE_MESSAGE_CONTROL,
        arg_count=1,
        extra=0,
        tail=0,
        trace_value=trace_value,
        args=[
            OperandRecord(
                ...
                length=4,
                offset=0,
                payload=encode_literal_value(MESSAGE_CONTROL_COMPILE_VALUE),
                uses_section3_payload=True,
            )
        ],
    )
...
def collect_oplist_text_entries(blob: YstbBinary, source_encoding: str) -> List[TextEntry]:
    ...
    def flush(end_idx: int) -> None:
        nonlocal pending_parts, pending_start, pending_offset, pending_length, text_id
    ...
    for inst in instructions:
        ...
        controls = decode_message_control_instruction(inst)
        if controls and pending_start is not None:
            pending_parts.extend(controls)
            continue
...
def collect_set_value_text_entries(blob: YstbBinary, source_encoding: str) -> List[TextEntry]:
    ...
    for inst in instructions:
        ...
        text_slots = SET_VALUE_TEXT_SLOTS.get(key)
        if not text_slots:
            continue
...
def collect_opcode36_marshaled_text_entries(blob: YstbBinary, source_encoding: str) -> List[TextEntry]:
    ...
    text_id = 0
    for inst in instructions:
        if inst.opcode != OPCODE_SET_RUNTIME_VALUE:
            continue
        if len(inst.args) < 2:
            continue

        value_arg = inst.args[1]
        if value_arg.kind != 3 or not value_arg.uses_section3_payload:
            continue

        text = decode_marshaled_string(value_arg.payload, source_encoding)
        if not text:
            continue
        ...
        out.append(
            TextEntry(
                ...
                kind=inst.opcode,
                flag=value_arg.slot,
                offset=value_arg.offset,
                length=value_arg.length,
                text=text,
                text_role=classify_text_role(text),
                extract_mode=EXTRACT_MODE_OP36_MARSHALED,
            )
        )
        text_id += 1
...
def is_legacy_split_message_span(
    instructions: Sequence[InstructionRecord],
    instr_start: int,
    instr_end: int,
) -> bool:
    if instr_end <= instr_start or instr_end > len(instructions):
        return False
    if instructions[instr_end - 1].opcode == OPCODE_MESSAGE_COMMIT:
        return True

    i = instr_end
    if i >= len(instructions) or instructions[i].opcode != OPCODE_MESSAGE_FORMAT:
        return False
    i += 1
    ...
    while i < len(instructions):
        ...
        if op == OPCODE_MESSAGE_TEXT:
            saw_text = True
            i += 1
            continue
        if op == OPCODE_MESSAGE_FORMAT:
            i += 1
            continue
        if op == OPCODE_MESSAGE_COMMIT:
            return saw_text
        return False
    return False
...
def build_message_instruction_sequence(
    ...
    first_text_inst: Optional[InstructionRecord] = None
    for inst in original_slice:
        if inst.opcode in {OPCODE_MESSAGE_TEXT, OPCODE_MESSAGE_FORMAT}:
            first_text_inst = inst
            break
    if first_text_inst is None:
        raise YstbError(f"text id {entry.text_id}: missing source text instruction template")

    trace_value = first_text_inst.trace_value
    has_commit = bool(original_slice) and original_slice[-1].opcode == OPCODE_MESSAGE_COMMIT
    commit_template = clone_instruction(original_slice[-1]) if has_commit else None
    out: List[InstructionRecord] = []

    for part_type, part in split_message_text_parts(new_text):
        if part_type == "text":
            if part == "":
                continue
            try:
                ...
                raise YstbError(f"text id {entry.text_id} encode failed with {target_encoding}: {exc}") from exc
            out.append(make_message_text_instruction(first_text_inst, text_bytes, trace_value))
        else:
            out.append(make_message_control_instruction(part, trace_value))

    if not out:
        out.append(make_message_text_instruction(first_text_inst, b"", trace_value))
...
def rebuild_with_oplist_text_edits(
    ...
    instructions = parse_instruction_records(blob)
    entries_by_start: Dict[int, TextEntry] = {}
    for entry in source_entries:
        if entry.instr_start is None or entry.instr_end is None:
            continue
        entries_by_start[entry.instr_start] = entry

    new_instructions: List[InstructionRecord] = []
    target_map: Dict[int, Optional[int]] = {}
    idx = 0
    while idx < len(instructions):
        entry = entries_by_start.get(idx)
        if entry is None:
        ...
        if entry.instr_end is None or entry.instr_end <= idx or entry.instr_end > len(instructions):
            raise YstbError(f"text id {entry.text_id}: invalid instruction span")

        original_slice = instructions[idx : entry.instr_end]
        if not is_legacy_split_message_span(instructions, idx, entry.instr_end):
        ...
        if (not force_reencode_all) and new_text == entry.text:
            ...
            rebuilt_slice = build_message_instruction_sequence(
                entry, original_slice, new_text, target_encoding, encoding_errors
            )
...
def classify_text_role(text: str) -> str:
    speaker, body = split_speaker_body(text)
...
def collect_text_entries(blob: YstbBinary, source_encoding: str) -> List[TextEntry]:
    oplist_entries = collect_oplist_text_entries(blob, source_encoding)
    if oplist_entries:
        return oplist_entries
    set_value_entries = collect_set_value_text_entries(blob, source_encoding)
    if set_value_entries:
        return set_value_entries
    return collect_opcode36_marshaled_text_entries(blob, source_encoding)
...
def should_encode_as_marshaled(extract_mode: str) -> bool:
    if extract_mode.startswith("set_value:"):
        return True
    return extract_mode == EXTRACT_MODE_OP36_MARSHALED
...
def rebuild_with_text_edits(
    ...
    if oplist_edit_ids:
        if plain_edit_ids:
            raise YstbError("cannot mix oplist text edits with range-based text edits in one compile pass")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="YSTB Tool V490")
