...
def _split_name_msg(item: Dict[str, object]) -> Tuple[str, str]:
    ...
    if speaker is None and isinstance(body, str):
        return "", body
...
def export_ystb_one(json_path: Path, out_txt: Path) -> int:
...
def apply_ystb_txt_payload(payload: Dict[str, object], edits: Dict[Tuple[int, str], str]) -> int:
    if str(payload.get("format", "")).upper() != "YSTB":
    ...
    for item in entries:
        ...
        old_name, old_msg = _split_name_msg(item)
...
def apply_ystb_one(base_json: Path, txt_file: Path, out_json: Path) -> int:
    payload = json.loads(base_json.read_text(encoding="utf-8-sig"))
    edits = parse_ystb_txt(txt_file)
    touched = apply_ystb_txt_payload(payload, edits)
...
def apply_ystb_cmd(args: argparse.Namespace) -> None:
    ...
    for t in txt_files:
        try:
            ...
            touched = apply_ystb_one(matched_json, t, out_file)
