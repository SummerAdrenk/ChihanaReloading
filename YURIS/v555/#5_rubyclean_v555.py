...
RUBY_BRACKET_PIPE_RE = re.compile(r"《([^《》]+?)[｜|][^《》]*》")
RUBY_DOUBLE_ANGLE_SLASH_RE = re.compile(r"≪([^≪≫]+?)[／/][^≪≫]*≫")
...
def strip_ruby_markup(text: str) -> str:
    ...
    while True:
        ...
        out = RUBY_BRACKET_PIPE_RE.sub(r"\1", out)
        out = RUBY_DOUBLE_ANGLE_SLASH_RE.sub(r"\1", out)
        if out == old:
            return out
