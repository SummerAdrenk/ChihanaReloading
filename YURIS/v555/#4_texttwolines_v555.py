...
NAME_DEF_TEXT_RE = re.compile(r"^[A-Za-z0-9\uFF21-\uFF3A\uFF41-\uFF5A\uFF10-\uFF19\u3041-\u3093\u30A1-\u30F6\u4E00-\u9FAF\u3005\u3006\u30F5\u30F6\u30FC\uFF06&\u30FB\uFF65\uFF0B+_\uFF3F\-\u3000 ]{1,24}$")
...
def _build_record_index(entries: List[Dict[str, object]]) -> Dict[int, Dict[str, object]]:
    out: Dict[int, Dict[str, object]] = {}
    for item in entries:
        if not isinstance(item, dict):
            continue
        try:
            rec = int(item.get("record_index", -1))
...
def _is_name_definition_entry(item: Dict[str, object], rec_index: Optional[Dict[int, Dict[str, object]]] = None) -> bool:
    try:
        t = int(item.get("type", -1))
        f = int(item.get("flag", -1))
    except Exception:  # noqa: BLE001
        return False

    if t not in (33, 34) or f != 3:
        return False
    ...
    # Reverse result: type 33/34 here are short quoted name literals in arg records.
    if any(ch in text for ch in ("\u300c", "\u300d", "\u3010", "\u3011", "\\", "/", '"', "@", "\u300a", "\u300b", "\u226a", "\u226b", "\u3002", "\u3001", "\uff01", "\uff1f", "!", "?")):
        return False

    if NAME_DEF_TEXT_RE.fullmatch(text) is None:
        return False
    ...
    if not isinstance(peer, dict):
        return False
    try:
        peer_t = int(peer.get("type", -1))
        peer_f = int(peer.get("flag", -1))
    except Exception:  # noqa: BLE001
        return False
    if peer_t != expect_type or peer_f != 3:
        return False
...
def _is_exportable_ystb_entry(item: Dict[str, object], rec_index: Optional[Dict[int, Dict[str, object]]] = None) -> bool:
    try:
        t = int(item.get("type", -1))
        f = int(item.get("flag", -1))
    except Exception:  # noqa: BLE001
    ...
    # Reverse result (v550): in-script visible text is mainly type=0/flag=0.
    # type 33/34 + flag 3 are arg literals; only paired same-text entries are name definitions.
    if t == 0 and f == 0:
        return True
    if t in (33, 34) and f == 3 and _is_name_definition_entry(item, rec_index):
        return True
    return False
...
def _split_name_msg(
    ...
    rec_index: Optional[Dict[int, Dict[str, object]]] = None,
    dumped_names: Optional[Set[str]] = None,
) -> Tuple[str, str]:
    ...
    def is_dumped_name(raw_name: str) -> bool:
        ...
        return bool(n) and (n in dumped_names)

    if isinstance(speaker, str):
        body_text = body if isinstance(body, str) else ""
        m_text = SPEAKER_RE.match(text)
        if m_text:
            return f"【{m_text.group(1)}】", m_text.group(2)
        return _normalize_name(speaker), body_text

    if _is_name_definition_entry(item, rec_index):
...
def _ordered_ystb_entries(entries: List[Dict[str, object]], payload: Dict[str, object]) -> List[Tuple[int, Dict[str, object]]]:
    ordered: List[Tuple[int, Dict[str, object]]] = []
    for item in entries:
        if not isinstance(item, dict):
            continue
        ordered.append((_ystb_entry_index(item, payload), item))
    ordered.sort(key=lambda x: x[0])
    return ordered


def _find_quote_continuation_pair(
    item: Dict[str, object],
    idx: int,
    rec_index: Dict[int, Dict[str, object]],
    idx_to_item: Dict[int, Dict[str, object]],
    next_idx_map: Dict[int, int],
) -> Optional[Tuple[int, Dict[str, object], str]]:
    if _detect_name_msg_style(item, rec_index) != "quote_open":
        return None

    next_idx = next_idx_map.get(idx)
    if next_idx is None:
        return None

    next_item = idx_to_item.get(next_idx)
    if not isinstance(next_item, dict):
        return None

    next_name, next_msg = _split_name_msg(next_item, rec_index)
    if next_name.strip():
        return None

    if _detect_name_msg_style(next_item, rec_index) != "plain":
        return None

    if not next_msg.strip().endswith("\u300d"):
        return None

    return next_idx, next_item, next_msg




def _normalize_ystb_role(raw: object) -> str:
    t = str(raw).strip().lower()
    if t in {"msg", "name", "sel", "choice", "menu"}:
        return "sel" if t == "choice" else t
    if t in {"system", "sys"}:
        return "system"
    return ""


def _line_type_from_role(role: str) -> str:
    if role == "system":
        return "sys"
    if role == "menu":
        return "menu"
...
def _role_from_line_type(line_type: str) -> str:
    t = line_type.strip().lower()
    if t == "sys":
        return "system"
    if t == "menu":
        return "menu"
...
def _collect_dumped_names_from_entries(entries: List[Dict[str, object]]) -> Set[str]:
    ...
    for item in entries:
        if not isinstance(item, dict):
            continue
        role = _normalize_ystb_role(item.get("text_role", ""))
        if role != "name":
            continue

        mode = str(item.get("extract_mode", "")).upper()
        if "NAME.MARK.SET" in mode:
            continue

        speaker = item.get("speaker")
        if isinstance(speaker, str) and speaker.strip():
            out.add(_normalize_name(speaker))
        ...
        if text.startswith("【") and text.endswith("】") and len(text) >= 2:
            text = text[1:-1]
        name_text = _normalize_name(text)
        if name_text:
            out.add(name_text)
    return out
...
def export_ystb_one(json_path: Path, out_txt: Path, shared_dumped_names: Optional[Set[str]] = None) -> int:
    ...
    rec_index = _build_record_index(entries)
    ...
    ordered = _ordered_ystb_entries(entries, payload)

    mark_source = chr(0x25C7)
    mark_trans = chr(0x25C6)

    lines: List[str] = []
    count = 0

    for idx_val, item in ordered:
        role = _normalize_ystb_role(item.get("text_role", ""))
        if not role:
            continue
        ...
        if role == "system":
            ...
            if not sys_text.strip():
                continue
            encoded = escape_single_line_text(sys_text)
            lines.append(f"{mark_source}{idx}{mark_source}sys{mark_source}{encoded}")
            lines.append(f"{mark_trans}{idx}{mark_trans}sys{mark_trans}{encoded}")
            lines.append("")
            count += 1
            continue

        if role == "menu":
            menu_text = str(item.get("text", ""))
            if not menu_text.strip():
                continue
            encoded = escape_single_line_text(menu_text)
            lines.append(f"{mark_source}{idx}{mark_source}menu{mark_source}{encoded}")
            lines.append(f"{mark_trans}{idx}{mark_trans}menu{mark_trans}{encoded}")
            lines.append("")
            count += 1
            continue

        if role == "sel":
            sel_text = str(item.get("text", ""))
            if not sel_text.strip():
                continue
            encoded = escape_single_line_text(sel_text)
            lines.append(f"{mark_source}{idx}{mark_source}sel{mark_source}{encoded}")
            lines.append(f"{mark_trans}{idx}{mark_trans}sel{mark_trans}{encoded}")
            lines.append("")
            count += 1
            continue
        ...
        if role == "name":
            speaker = item.get("speaker")
            if isinstance(speaker, str) and speaker.strip():
                name_text = speaker.strip()
            else:
                name_text = str(item.get("text", "")).strip()
            if not name_text:
                continue
            n = escape_single_line_text(name_text)
            lines.append(f"{mark_source}{idx}{mark_source}name{mark_source}{n}")
            lines.append(f"{mark_trans}{idx}{mark_trans}name{mark_trans}{n}")
            lines.append("")
            count += 1
            continue

        name, msg = _split_name_msg(item, rec_index, active_dumped_names)
        ...
        if msg.strip():
            ...
            lines.append(f"{mark_source}{idx}{mark_source}msg{mark_source}{m}")
            lines.append(f"{mark_trans}{idx}{mark_trans}msg{mark_trans}{m}")
            lines.append("")
            count += 1
...
def export_ystb_cmd(args: argparse.Namespace) -> None:
    ...
    if inp.is_file():
        ...
        try:
            ...
            if n == 0:
                print(f"[SKIP] {inp}: no exportable name/msg/sel/sys/menu entries")
            else:
    ...
    for p in json_files:
        try:
            ...
            if n == 0:
                print(f"[SKIP] {p}: no exportable name/msg/sel/sys/menu entries")
                skip += 1
            else:
...
def parse_ystb_txt(path: Path) -> Dict[Tuple[int, str], str]:
    ...
    for line in lines:
        ...
        if ms:
            role = _normalize_ystb_role(_role_from_line_type(ms.group(2)))
            if not role:
                pending = None
                continue
            key = (int(ms.group(1), 16), role)
            pending = key
            continue

        mt = YSTB_TRANS_TYPED_RE.match(line)
        if mt:
            role = _normalize_ystb_role(_role_from_line_type(mt.group(2)))
            if not role:
                pending = None
                continue
            key = (int(mt.group(1), 16), role)
...
def _write_name_msg_to_item(item: Dict[str, object], style: str, new_name: str, new_msg: str) -> None:
    if new_name.strip():
        ...
        elif style in {"quote_open", "paren", "paren_open"}:
            item.pop("speaker", None)
            item.pop("body", None)
            item["text"] = f"{speaker}{new_msg}"
        else:
...
def _set_item_text_by_role(item: Dict[str, object], role: str, new_text: str) -> None:
    item["text"] = new_text

    if role == "msg":
        m = SPEAKER_RE.match(new_text)
        if m:
            item["speaker"] = m.group(1)
            item["body"] = m.group(2)
        else:
            item["speaker"] = None
            item["body"] = new_text
...
def apply_ystb_txt_payload(
    ...
    for idx, item in ordered:
        ...
        if role == "system":
            key_sys = (idx, "system")
            if key_sys not in edits:
                continue
            old_text = str(item.get("text", ""))
            new_text = edits[key_sys]
            ...
            if "body" in item:
                item["body"] = new_text
            item["text_role"] = "system"
            touched += 1
            continue

        if role == "menu":
            key_menu = (idx, "menu")
            if key_menu not in edits:
                continue
            old_text = str(item.get("text", ""))
            new_text = edits[key_menu]
            if new_text == old_text:
                continue
            item["text"] = new_text
            if "speaker" in item:
        ...
        if role == "sel":
            key_sel = (idx, "sel")
            if key_sel not in edits:
                continue
            old_text = str(item.get("text", ""))
            new_text = edits[key_sel]
            if new_text == old_text:
                continue
            item["text"] = new_text
            if "speaker" in item:
        ...
        if role == "name":
            key_name = (idx, "name")
            if key_name not in edits:
                continue
            old_name, _old_msg = _split_name_msg(item, rec_index, active_dumped_names)
            if not old_name.strip():
                old_name = _normalize_name(str(item.get("text", "")))
            new_name = edits[key_name]
            if new_name == old_name:
                continue

            speaker = _speaker_from_name(new_name)
            # IDA??????case 0x2B / key="es.CHAR.NAME" ??????????????????
            item["speaker"] = None
            item["body"] = speaker
            item["text"] = speaker
            item["text_role"] = "name"
        ...
        old_name, old_msg = _split_name_msg(item, rec_index, active_dumped_names)
        ...
        if name_for_compare != old_name or (has_msg_edit and new_msg != old_msg):
            style = _detect_name_msg_style(item, rec_index, active_dumped_names)
...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] in {"decompile", "compile"}:
        raise SystemExit(
            "decompile/compile are in YSTB_Tool_v555.py; use: "
            "python YSTB_Tool_v555.py decompile <input_ybn> <out_dir>"
        )
    ...
    pe = sub.add_parser("export-ystb", help="export YSTB JSON to two-line name/msg/sel/sys/menu txt")
