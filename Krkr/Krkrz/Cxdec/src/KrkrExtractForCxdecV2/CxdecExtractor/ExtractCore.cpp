...
namespace Engine
{
	...
	void ExtractCore::ExtractPackage(const std::wstring& packageFileName)
	{
		...
		if (!entries.empty())
		{
			...
			// 多线程实现
			const uint32_t totalCount = static_cast<uint32_t>(entries.size());

			// 重置多线程状态
			mProducerDone.store(false);
			mWrittenCount.store(0);
			mErrorCount.store(0);
			...
			this->mLogger.WriteLine(L"[%s] 开始解包: %u 个文件, %u 个写入线程",packageFileName.c_str(), totalCount, threadCount);

			std::vector<std::thread> workers;
			workers.reserve(threadCount);
			for (unsigned int i = 0; i < threadCount; ++i)
			{
				workers.emplace_back(&ExtractCore::WriterThreadFunc, this);
			}

			// 主线程: 读取流数据，写文件表，推送写入任务
			uint32_t streamReadCount = 0;
			for (FileEntry& entry : entries)
			{
				...
				if (IStream* stream = this->CreateStream(entry, packageFileName))
				{
					...
					std::wstring relativePath(&arcOutputPath.c_str()[this->mExtractDirectoryPath.length() + 1u]);

					// 先检查流长度，空文件与原版一致仅记录
					unsigned long long streamSize = StreamUtils::IStreamEx::Length(stream);
					if (streamSize > 0)
					{
						// 从流中读取全部数据到内存
						std::vector<uint8_t> buffer;
						bool readOk = this->ReadStreamToBuffer(stream, buffer);
						stream->Release();
						...
						if (readOk && !buffer.empty())
						{
							// 构建写入任务
							WriteTask task;
							task.outputPath = std::move(arcOutputPath);
							task.relativePath = std::move(relativePath);
							task.data = std::move(buffer);

							// 推入队列 (有界队列，满时阻塞等待消费者腾出空间)
							{
								std::unique_lock<std::mutex> lock(mQueueMutex);
								mQueueNotFull.wait(lock, [this] { return mTaskQueue.size() < MaxQueueSize; });
				...
				// 每 500 个文件输出一次进度
				if (streamReadCount % 500 == 0)
				{
					std::lock_guard<std::mutex> logLock(mLogMutex);
					this->mLogger.WriteLine(L"读取进度: %u/%u, 已写入: %u, 错误: %u",
											streamReadCount, totalCount, mWrittenCount.load(), mErrorCount.load());
				}
			}

			// 通知所有消费者: 生产者已完成
			mProducerDone.store(true);
			...
			uint32_t written = mWrittenCount.load();
			uint32_t errors = mErrorCount.load();
			uint32_t skipped = totalCount - written - errors;

			this->mLogger.WriteLine(L"[%s] 解包完成: 总计 %u, 成功 %u, 跳过 %u, 错误 %u",
									packageFileName.c_str(), totalCount, written, skipped, errors);

			wchar_t msgBuf[256];
			swprintf_s(msgBuf, L"%s 提取完成\n总计: %u  成功: %u  跳过: %u  错误: %u  线程: %u",
					   packageFileName.c_str(), totalCount, written, skipped, errors, threadCount);
			::MessageBoxW(nullptr, msgBuf, L"信息", MB_OK);
	...
	bool ExtractCore::ReadStreamToBuffer(IStream* stream, std::vector<uint8_t>& buffer)
	{
		...
		return StreamUtils::IStreamEx::Read(stream, buffer.data(), static_cast<ULONG>(size)) == static_cast<ULONG>(size);
	}


	void ExtractCore::WriterThreadFunc()
	{
		while (true)
		{
			WriteTask task;
			{
				std::unique_lock<std::mutex> lock(mQueueMutex);
				mQueueNotEmpty.wait(lock, [this] { return !mTaskQueue.empty() || mProducerDone.load(); });

				if (mTaskQueue.empty() && mProducerDone.load())
					break;

				task = std::move(mTaskQueue.front());
				mTaskQueue.pop();
			}
			mQueueNotFull.notify_one();

			// 创建目录
			std::wstring outputDir = Path::GetDirectoryName(task.outputPath);
			if (!outputDir.empty())
			{
			...
			// 写入文件
			if (File::WriteAllBytes(task.outputPath, task.data.data(), task.data.size()))
			{
				mWrittenCount.fetch_add(1);
				std::lock_guard<std::mutex> logLock(mLogMutex);
				this->mLogger.WriteLine(L"Extract Successed: %s", task.relativePath.c_str());
			}
			else
			{
				mErrorCount.fetch_add(1);
				std::lock_guard<std::mutex> logLock(mLogMutex);
				this->mLogger.WriteLine(L"Write Error: %s", task.relativePath.c_str());
			}
		}
	}
