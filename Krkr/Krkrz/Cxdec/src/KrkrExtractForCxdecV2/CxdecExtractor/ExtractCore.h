...
namespace Engine
{
	...
	private:
        ...
        // ===== 多线程相关成员 =====
        static constexpr size_t MaxQueueSize = 64;          //任务队列上限 (控制内存占用)

        std::mutex mQueueMutex;                              //任务队列互斥锁
        std::condition_variable mQueueNotEmpty;              //队列非空条件
        std::condition_variable mQueueNotFull;               //队列未满条件
        std::queue<WriteTask> mTaskQueue;                    //待写入任务队列
        std::atomic<bool> mProducerDone{ false };            //生产者完成标志
        std::atomic<uint32_t> mWrittenCount{ 0 };            //已写入计数
        std::atomic<uint32_t> mErrorCount{ 0 };              //错误计数
        std::mutex mLogMutex;                                //日志互斥锁
	...
	private:
        ...
        /// <summary>
        /// 从流中读取全部数据到缓冲区 (主线程调用，含 SimpleEncrypt 解密)
        /// </summary>
        /// <param name="stream">资源流</param>
        /// <param name="buffer">输出缓冲区</param>
        /// <returns>True读取成功 False失败或空文件</returns>
        bool ReadStreamToBuffer(IStream* stream, std::vector<uint8_t>& buffer);
